...
p1 <- p1 + labs(x= "Shift (negative) in cm", y= "Probability of correct decisions in %") + theme_minimal()
...
fake.dat <- ddply(data.frame(sample=1:1000), .(sample), sampling)
...
#rounded <- format(round(fake.thresholds, 1), nsmall = 2)
#rounded$thre

png("/home/flip/Documents/project/HapStick/negative.png")
